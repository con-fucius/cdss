...
class TestClinicalRulesLoading:
    def test_rules_load_from_yaml(self):
        rules_path = str(Path(__file__).resolve().parents[1] / "app" / "rules" / "clinical_rules.yaml")
    ...
    def test_swahili_terms_present(self):
        ...
        for rule in data.get("rules", []):
            ...
            if any(any(ord(c) > 127 for c in s) or s in [
                "kushindwa kupumua", "kimefulia", "kitu kimekwama",
                "maumivu ya kifua", "kifua kinauma", "kizunguzungu",
                "degedege", "mshtuko", "kipindupindu", "moyo haupigi",
                "moyo umesimama", "kuzimia", "hakuna", "kutoka damu nyingi",
                "damu inatoka", "majeraha ya kichwa", "mifupa imevunjika",
                "alijichoma", "alianguka", "anajichanganya", "kichwa kinauma sana",
                "kohoa damu", "kifua kinafunga", "anashindwa kupumua",
                "hawezi kupumua", "pumua ina shida", "anapumua kwa shida",
                "kiwangulizi", "moyo unapiga haraka", "moyo unapiga polepole",
                "anakwaruka", "pumua", "kupoteza fahamu", "hajui chochote",
                "amezimia", "accident ya gari", "gari imegonga",
                "boda boda accident", "baridi", "anatetemeka",
                "joto kali", "umeme", "imeamua maji", "sumu",
                "damu katika ujauzito", "degedege wakati wa ujauzito",
                "kitovu kimetoka", "kujifungua", "mtoto anashindwa kupumua",
                "mtoto anapumua haraka", "joto mtoto", "mtoto anavutavuta",
            ] for s in synonyms):
                swahili_count += 1
...
class TestExtractor:
    ...
    def test_extracts_known_terms(self):
        from app.pipeline.extractor import extract_keywords, _rules_cache, _compiled_patterns
    ...
    def test_empty_input_returns_fallback(self):
        ...
        rules = [{"term": "chest pain", "synonyms": [], "category": "CARDIOVASCULAR", "severity_weight": 0.8}]
        rules_path = self._make_rules_file(rules)
...
class TestRanker:
    ...
    def test_gcs_3_produces_p1(self):
        ...
        keywords = [
            ExtractedKeyword(
                text="unconscious", category=ClinicalCategory.NEUROLOGICAL,
    ...
    def test_shock_index_above_1_produces_critical(self):
        ...
        keywords = [
            ExtractedKeyword(
                text="bleeding", category=ClinicalCategory.CARDIOVASCULAR,
                severity_modifiers=[],
            )
        ]
        ranking, shock_index = rank_diagnoses(
            keywords, hr=120, sbp=90,
            rules=[{"category": "CARDIOVASCULAR", "severity_weight": 0.8}]
        )
    ...
    def test_acvpu_to_gcs_mapping(self):
        ...
        from app.pipeline.ranker import rank_diagnoses
        keywords = [
            ExtractedKeyword(text="unconscious", category=ClinicalCategory.NEUROLOGICAL)
        ]
        ranking, _ = rank_diagnoses(
            keywords, acvpu="unresponsive",
...
class TestResolver:
    ...
    def test_l4_fallback_returns_none_for_unknown(self):
        from app.pipeline.resolver import _l4_fallback
        result = _l4_fallback("unknown_term_xyz", [{"term": "chest pain", "category": "CARDIOVASCULAR"}])
