...
# C. Breakout Indicator: 50-Day High (The Stress Shield)
df['High_50'] = df['Gold_High'].rolling(50).max().shift(1)

# D. Exit Indicator: 20-Day Low
df['Low_20'] = df['Gold_Low'].rolling(20).min().shift(1)

# E. Master Setup: Secular Bull + Vol Expanding + Closed above 50-day high
df['Setup_Active'] = df['Secular_Bull'] & df['Vol_Expanding'] & (df['Gold_Close'] > df['High_50'])

df = df.dropna()

# ==========================================
# 3. BACKTEST ENGINE (V11)
# ==========================================
def run_backtest_v11(df, start_equity, risk_pct, entry_slippage, exit_slippage, use_stress_exit=False):
    ...
    print("=" * 50)
    print(f"V11 SYNTHESIS | Start: ${start_equity} | Risk: {risk_pct*100}% | {stress_tag}")
    print("=" * 50)
...
# ==========================================
print("\n--- V11 Realistic Test ---")
run_backtest_v11(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=False)

print("\n--- V11 Stress Test (Modeling $20 Exit Spread) ---")
run_backtest_v11(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=True)