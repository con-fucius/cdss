...
# ==========================================
# 2. FEATURE ENGINEERING (V10 - FREQUENCY)
# ==========================================
# A. Regime Filter: Close > 200-day SMA (KEPT)
df['SMA_200'] = df['Gold_Close'].rolling(200).mean()
df['Secular_Bull'] = df['Gold_Close'] > df['SMA_200']

# B. Volatility Expansion: ATR(14) rising over 5 days (RELAXED)
...
df['ATR_14_5d_ago'] = df['ATR_14'].shift(5)
df['Vol_Expanding'] = df['ATR_14'] > df['ATR_14_5d_ago']

# C. Breakout Indicator: 20-Day High (FASTER)
df['High_20'] = df['Gold_High'].rolling(20).max().shift(1)

# D. Exit Indicator: 10-Day Low (FASTER)
df['Low_10'] = df['Gold_Low'].rolling(10).min().shift(1)

# E. Master Setup: Secular Bull + Vol Expanding + Closed above 20-day high
df['Setup_Active'] = df['Secular_Bull'] & df['Vol_Expanding'] & (df['Gold_Close'] > df['High_20'])

df = df.dropna()

# ==========================================
# 3. BACKTEST ENGINE (V10)
# ==========================================
def run_backtest_v10(df, start_equity, risk_pct, entry_slippage, exit_slippage, use_stress_exit=False):
    ...
    for i in range(1, len(df)):
        ...
        if in_trade:
            ...
            # 2. Trend Exit (Close < 10-day Low)
            elif prev_row['Gold_Close'] < prev_row['Low_10']:
        ...
        if not in_trade and prev_row['Setup_Active']:
            buy_stop_price = prev_row['High_20'] + entry_slippage
            
            if row['Gold_High'] >= buy_stop_price:
                entry_price = buy_stop_price
                stop_distance = max(2.0 * row['ATR_14'], 2.00) 
    ...
    print("=" * 50)
    print(f"V10 FREQUENCY ENGINE | Start: ${start_equity} | Risk: {risk_pct*100}% | {stress_tag}")
    print("=" * 50)
...
# ==========================================
print("\n--- V10 Realistic Test ---")
run_backtest_v10(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=False)

print("\n--- V10 Stress Test (Modeling $20 Exit Spread) ---")
run_backtest_v10(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=True)