...
# ==========================================
# 2. FEATURE ENGINEERING (V12 - THE SQUEEZE)
# ==========================================
...
# Squeeze Fires when it breaks out upwards
df['Squeeze_Fired'] = df['Gold_Close'] > df['KC_Upper']

# E. Setup: Secular Bull + Squeeze Fired
df['Setup_Active'] = df['Secular_Bull'] & df['Squeeze_Fired']

# F. Exit: 10-Day Low
df['Low_10'] = df['Gold_Low'].rolling(10).min().shift(1)
df['Exit_Signal'] = df['Gold_Close'] < df['Low_10']

df = df.dropna()

# ==========================================
# 3. BACKTEST ENGINE (V12)
# ==========================================
def run_backtest_v12(df, start_equity, risk_pct, entry_slippage, exit_slippage, use_stress_exit=False):
    ...
    for i in range(1, len(df)):
        ...
        if not in_trade and prev_row['Setup_Active']:
            # Buy Stop at yesterday's High
            buy_stop_price = prev_row['Gold_High'] + entry_slippage
            
            if row['Gold_High'] >= buy_stop_price:
                entry_price = buy_stop_price
                # Stop is 2.5x ATR, floored at $3.00
                stop_distance = max(2.5 * prev_row['ATR_14'], 3.00) 
    ...
    print("=" * 50)
    print(f"V12 SQUEEZE | Start: ${start_equity} | Risk: {risk_pct*100}% | {stress_tag}")
    print("=" * 50)
...
# ==========================================
print("\n--- V12 Realistic Test ---")
run_backtest_v12(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=False)

print("\n--- V12 Stress Test (Modeling $20 Exit Spread) ---")
run_backtest_v12(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=True)