...
df['ATR_100'] = true_range.rolling(100).mean()
df['Vol_Expanding'] = df['ATR_20'] > df['ATR_100']

# C. Breakout Indicator: 50-Day High
...
# ==========================================
# 3. BACKTEST ENGINE (V9)
# ==========================================
def run_backtest_v9(df, start_equity, risk_pct, entry_slippage, exit_slippage, use_stress_exit=False):
    ...
    for i in range(1, len(df)):
        ...
        if not in_trade and prev_row['Setup_Active']:
            ...
            if row['Gold_High'] >= buy_stop_price:
                ...
                stop_distance = max(2.0 * row['ATR_20'], 2.00) 
    ...
    print("=" * 50)
    print(f"V9 FINAL ENGINE | Start: ${start_equity} | Risk: {risk_pct*100}% | {stress_tag}")
    print("=" * 50)
...
# ==========================================
print("\n--- V9 Realistic Test ---")
run_backtest_v9(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=False)

print("\n--- V9 Stress Test (Modeling $20 Exit Spread) ---")
run_backtest_v9(df, start_equity=1500, risk_pct=0.02, entry_slippage=0.50, exit_slippage=0.50, use_stress_exit=True)