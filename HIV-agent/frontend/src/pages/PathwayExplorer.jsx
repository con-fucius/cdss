...
function StepIcon({ status }) {
  switch (status) {
    case 'completed': return <CheckCircle2 size={18} className="pathway-step-icon pathway-step-icon--completed" />;
    case 'current':   return <MapPin size={18} className="pathway-step-icon pathway-step-icon--current" />;
    case 'blocked':   return <XCircle size={18} className="pathway-step-icon pathway-step-icon--blocked" />;
    default:          return <Circle size={18} className="pathway-step-icon pathway-step-icon--pending" />;
  }
}
    ...
    >
      ...
      <div className="pathway-step__body">
        <div className="pathway-step__header">
          ...
          <span className={`pathway-step__badge pathway-step__badge--${event.status}`}>{event.status}</span>
        </div>
        {event.guideline_ref && (
        ...
        {event.blocking_inputs?.length > 0 && (
          <div className="pathway-step__blocking">
            ...
            {event.blocking_inputs.map(inp => <span key={inp} className="pathway-step__blocking-tag">{inp}</span>)}
          </div>
        )}
      </div>
    </motion.div>
...
export default function PathwayExplorer({ patientRefHash }) {
  ...
  const visiblePathways = filterDisease === 'all'
    ? pathways
    : pathways.filter(p => p.disease === filterDisease);

  const runPathway = useCallback((pathwayId) => {
    ...
    <div className="pathway-explorer">
      <div className="pathway-explorer__header">
        ...
        <div>
          ...
          <p className="pathway-explorer__subtitle">Guideline-anchored treatment pathways evaluated against patient state</p>
        </div>
      </div>

      {!patientRefHash && (
        <div className="pathway-notice">
          <AlertTriangle size={16} /> Load a patient in the Patient Context panel to evaluate pathways.
        </div>
      )}
      ...
      <div className="pathway-layout">
        ...
        <div className="pathway-list-panel">
          ...
          {loadingList && <div className="pathway-loading"><Loader2 size={20} className="ddx-spin" /> Loading pathways…</div>}
          ...
          <div className="pathway-list">
            {visiblePathways.map(pw => (
              <button
                ...
                onClick={() => pw.step_count > 0 ? runPathway(pw.pathway_id) : null}
                disabled={!patientRefHash || pw.step_count === 0}
                title={pw.step_count === 0 ? 'Pathway definition pending' : pw.target_population}
              >
                <div className="pathway-item__info">
                  <span className="pathway-item__name">{pw.pathway_name}</span>
                  <span className="pathway-item__disease">{pw.disease.replace('_', ' ')}</span>
                </div>
                <div className="pathway-item__meta">
                  {pw.step_count > 0
                    ? <span className="pathway-item__steps">{pw.step_count} steps</span>
                    : <span className="pathway-item__pending">Pending</span>
                  }
                  {selectedId !== pw.pathway_id && pw.step_count > 0 && patientRefHash && <ChevronRight size={14} />}
                  {selectedId === pw.pathway_id && isRunning && <Loader2 size={14} className="ddx-spin" />}
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="pathway-detail-panel">
          {!selectedId && (
            <div className="pathway-empty">
              ...
              <p>Select a pathway from the list to evaluate it against the current patient.</p>
            </div>
          )}
          ...
          {selectedId && (
            <>
              <div className="pathway-detail__header">
                <h2 className="pathway-detail__title">{selectedPathway?.pathway_name}</h2>
                <p className="pathway-detail__population">{selectedPathway?.target_population}</p>
              </div>
                ...
                <div className="pathway-contraindications">
                  {contraindications.map((c, i) => (
                    <div key={i} className="pathway-contraindication">
                      <ShieldAlert size={14} />
                      <strong>{c.drug}</strong> contraindicated with <strong>{c.condition}</strong>
                      {c.source_ref && <span className="pathway-contraindication__ref"> ({c.source_ref})</span>}
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="pathway-steps">
                <AnimatePresence>
                  {steps.map((step, i) => <StepCard key={step.step_id || i} event={step} />)}
                </AnimatePresence>

                {isRunning && steps.length > 0 && (
                  <div className="pathway-loading-inline"><Loader2 size={14} className="ddx-spin" /> Evaluating next step…</div>
                )}
              </div>

              {summary && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="pathway-summary">
                  <h3 className="pathway-summary__title">Summary</h3>
                  <div className="pathway-summary__grid">
                    <div className="pathway-summary__stat">
                      <CheckCircle2 size={16} className="pathway-step-icon--completed" />
                      <span>{summary.completed_steps?.length || 0} Completed</span>
                    </div>
                    {summary.current_step && (
                      <div className="pathway-summary__stat">
                        <MapPin size={16} className="pathway-step-icon--current" />
                    ...
                    {summary.monitoring_due?.length > 0 && (
                      <div className="pathway-summary__monitoring">
                        ...
                        Monitoring due: {summary.monitoring_due.map(m => m.drug).join(', ')}
                      </div>
                    )}
                  </div>
                  {summary.next_actions?.length > 0 && (
                    <div className="pathway-summary__actions">
                      <p className="pathway-summary__actions-label">Next required data:</p>
                      {summary.next_actions.map(a => <span key={a} className="pathway-summary__action-tag">{a}</span>)}
                    </div>
                  )}
                </motion.div>
              )}
