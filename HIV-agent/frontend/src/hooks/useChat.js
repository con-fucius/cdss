...
const summarizeTitle = (text) => {
  ...
  return cleaned.length > 64 ? `${cleaned.slice(0, 61)}...` : cleaned || 'Clinical session';
};

export function useChat(sessionId, initialQuery, patientContext, _settings, patientRefHash) {
  ...
  const [feedbackGiven, setFeedbackGiven] = useState(() => loadFromStorage(STORAGE_KEYS.feedbackGiven) || {});
  const [pinnedMessages, setPinnedMessages] = useState(() => loadFromStorage(STORAGE_KEYS.pinned) || []);
  ...
  useEffect(() => { saveToStorage(STORAGE_KEYS.feedbackGiven, feedbackGiven); }, [feedbackGiven]);
  useEffect(() => { saveToStorage(STORAGE_KEYS.pinned, pinnedMessages); }, [pinnedMessages]);
  ...
  const addAgentAction = (text, detail = '') => setAgentActions(prev => [...prev, { text, detail, done: false, time: new Date().toISOString() }]);
  const completeAgentAction = (text) => setAgentActions(prev => prev.map(a => a.text === text ? { ...a, done: true } : a));

  const handleSend = useCallback(async (text, isRegeneration = false) => {
    if (!text.trim() || isLoading) return;

    if (!isRegeneration) {
      const userMsg = { role: 'user', content: text, timestamp: new Date().toISOString(), id: generateId() };
    ...
    try {
      const body = { session_id: sessionId, message: text, context: patientContext };
      ...
      if (!response.ok) throw new Error(`Failed to connect to clinical agent (HTTP ${response.status})`);
      if (!response.body) throw new Error('Clinical agent response stream is unavailable');
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: '', timestamp: new Date().toISOString(), sources: [], concepts: [], triples: [], interactions: [], reasoning: [], drugInteractionStatus: null, id: msgId }]);
          ...
          for (const payload of dataLines) {
            ...
            try {
              ...
              if (data.type === 'loading' || data.type === 'activity') {
                ...
                addAgentAction(`${data.tool_name}: ${data.query || 'running'}`, data.detail || '');
                ...
                if (data.latency_ms) addAgentAction('Response completed', `${Math.round(data.latency_ms)} ms`);
                ...
                setMessages(prev => {
                  const newMessages = [...prev];
                  newMessages[newMessages.length - 1].concepts = data.concepts || [];
                  return newMessages;
                });
              } else if (data.type === 'evidence') {
                setMessages(prev => {
                  const newMessages = [...prev];
                  newMessages[newMessages.length - 1].triples = data.triples || [];
                  return newMessages;
                });
              } else if (data.type === 'drug_interactions') {
                setMessages(prev => {
                  const newMessages = [...prev];
                  newMessages[newMessages.length - 1].interactions = data.interactions || [];
                  newMessages[newMessages.length - 1].drugInteractionStatus = data;
                  return newMessages;
                });
                ...
                setMessages(prev => {
                  ...
                  last.clinicalScores = [...(last.clinicalScores || []), ...(data.scores || [])];
                  return newMessages;
                });
                ...
                setMessages(prev => {
                  const newMessages = [...prev];
                  const last = newMessages[newMessages.length - 1];
                  last.reasoning = [...(last.reasoning || []), data.summary || data.content || 'Provider returned reasoning metadata'];
                  return newMessages;
                });
                addAgentAction('Reasoning summary', data.summary || data.content || 'Provider returned reasoning metadata');
            ...
            } catch (_error) { /* ignore chunking noise */ }
            }
          }
        }
      }
  ...
  const submitFeedback = useCallback(async ({ message, feedbackType, note = '', correction = '' }) => {
    const sourcesUsed = (message.sources || []).map(source =>
      typeof source === 'string' ? source : source.source || source.chunk_id || ''
    ).filter(Boolean);
  ...
  return {
    ...
    currentConvId, setCurrentConvId: selectConversation,
