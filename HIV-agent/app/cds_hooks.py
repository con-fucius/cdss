...
class CDSHooksHandler:
    async def handle_patient_view(self, context: Dict[str, Any]) -> List[CDSCard]:
        try:
            ...
            patient_ref = _hash_patient_ref({"patient_ref": patient_id}) if "patientId" in context else patient_id
            ...
            for score in scores:
                if score.get("alert_level") == "CRITICAL":
                    cards.append(CDSCard(
                        ...
                        suggestions=[],
                        override_reasons=[
                            "clinically_irrelevant", "already_actioned", "patient_specific_exception",
                            "incorrect_alert", "duplicate"
                        ]
                    ))
            ...
            for med in active_meds:
                ...
                if not med_name:
                    continue
                edges = await query_evidence_graph_db(disease="general", entity_name=med_name, top_k=5)
                for edge in edges:
    ...
    async def handle_medication_prescribe(self, context: Dict[str, Any]) -> List[CDSCard]:
        try:
            ...
            from .logs import _hash_patient_ref
            patient_ref = _hash_patient_ref({"patient_ref": patient_id}) if patient_id else "unknown"
            patient_state = await get_patient_state(patient_ref)
            active_meds = [m.get("name") for m in patient_state.get("active_medications", []) if m.get("name")]
                ...
                for inter in interactions_res.get("interactions", []):
                    ...
                    if indicator in ("critical", "warning"):
                        cards.append(CDSCard(
                            ...
                            suggestions=[],
                            override_reasons=["clinically_irrelevant", "patient_specific_exception"]
                        ))
