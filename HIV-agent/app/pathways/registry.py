...
def _has_creatinine(state: dict[str, Any]) -> bool:
    return any(lab.get("lab_type") == "creatinine" for lab in state.get("labs", []))
...
def _has_art(state: dict[str, Any]) -> bool:
    ...
    return any(
        med.get("name")
        for med in meds
        if "art" in med.get("name", "").lower()
        or "tdf" in med.get("name", "").lower()
        or "dtg" in med.get("name", "").lower()
    )
...
PATHWAY_REGISTRY: dict[str, ClinicalPathway] = {
    ...
    "tb_ds_initiation": ClinicalPathway(
        ...
        steps=[
            PathwayStep(
                ...
                completion_criteria=lambda s: any(
                    l.get("lab_type") in ["alt", "ast", "bilirubin"] for l in s.get("labs", [])
                ),
            ...
            PathwayStep(
                ...
                completion_criteria=lambda s: any(
                    "rifampicin" in m.get("name", "").lower()
                    for m in s.get("active_medications", [])
                ),
    ...
    "t2dm_initial": ClinicalPathway(
        ...
        entry_criteria=lambda s: "t2dm" in s.get("active_diagnoses", [])
        or "diabetes" in s.get("active_diagnoses", []),
        steps=[
            PathwayStep(
                step_id="t2dm_1",
                name="Baseline HbA1c",
                ...
                completion_criteria=lambda s: any(
                    l.get("lab_type") == "hba1c" for l in s.get("labs", [])
                ),
    ...
    # 9 Shell Pathways
    "hiv_pmtct": ClinicalPathway(
        pathway_id="hiv_pmtct",
        disease="hiv",
        name="HIV PMTCT",
        target_population="Pregnant women living with HIV",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "tb_mdr": ClinicalPathway(
        pathway_id="tb_mdr",
        disease="tb",
        name="MDR-TB Management",
        target_population="Patients with Rifampicin-resistant TB",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "t1dm_pediatric": ClinicalPathway(
        pathway_id="t1dm_pediatric",
        disease="diabetes",
        name="Pediatric T1DM",
        target_population="Children diagnosed with Type 1 Diabetes",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "cvd_hypertension": ClinicalPathway(
        pathway_id="cvd_hypertension",
        disease="cvd",
        name="Uncomplicated Hypertension",
        target_population="Adults with blood pressure >140/90",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "cvd_heart_failure": ClinicalPathway(
        pathway_id="cvd_heart_failure",
        disease="cvd",
        name="Chronic Heart Failure",
        target_population="Patients with symptomatic heart failure",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "malaria_uncomplicated": ClinicalPathway(
        pathway_id="malaria_uncomplicated",
        disease="malaria",
        name="Uncomplicated Malaria",
        target_population="Patients with positive mRDT/microscopy and no danger signs",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "malaria_severe": ClinicalPathway(
        pathway_id="malaria_severe",
        disease="malaria",
        name="Severe Malaria",
        target_population="Patients with severe malaria criteria",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "mh_depression": ClinicalPathway(
        pathway_id="mh_depression",
        disease="mental_health",
        name="Major Depressive Disorder",
        target_population="Patients with PHQ-9 > 9",
        entry_criteria=lambda s: True,
        steps=[],
    ),
    "mh_psychosis": ClinicalPathway(
        pathway_id="mh_psychosis",
        disease="mental_health",
        name="Acute Psychosis",
        target_population="Patients presenting with acute psychotic symptoms",
        entry_criteria=lambda s: True,
        steps=[],
    ),
}
