...
class PathwayRunner:
    async def run(
        self, pathway_id: str, patient_ref: str, patient_state: dict[str, Any]
    ) -> AsyncIterator[dict]:
        ...
        for i, step in enumerate(pathway.steps, 1):
            ...
            if step.contraindication_check:
                ...
                edges = await query_evidence_graph_db(
                    disease=pathway.disease, entity_name=step.contraindication_check, top_k=5
                )
                for edge in edges:
