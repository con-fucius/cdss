...
class ClinicalScorer:
    ...
    def news2(vitals: dict[str, Any]) -> dict[str, Any]:
        ...
        score += rr_score
        if rr_score > 0:
            triggers.append(f"RR = {rr} (score {rr_score})")
        ...
        score += spo2_score
        if spo2_score > 0:
            triggers.append(f"SpO2 = {spo2}% (score {spo2_score})")
        ...
        if supp_o2:
            ...
            score += o2_score
            triggers.append("On supplemental O2 (score 2)")
        else:
        ...
        score += bp_score
        if bp_score > 0:
            triggers.append(f"BP = {bp} (score {bp_score})")
        ...
        score += hr_score
        if hr_score > 0:
            triggers.append(f"HR = {hr} (score {hr_score})")
        ...
        score += temp_score
        if temp_score > 0:
            triggers.append(f"Temp = {temp} (score {temp_score})")
    ...
    @staticmethod
    def who_hiv_stage(clinical_features: list[str], cd4: float | None = None) -> dict[str, Any]:
        ...
        if stage == 1:
            criteria_not_met_for_next_stage = stage_2_criteria
        elif stage == 2:
            criteria_not_met_for_next_stage = stage_3_criteria
        elif stage == 3:
            criteria_not_met_for_next_stage = stage_4_criteria
    ...
    def child_pugh(labs: dict[str, Any], clinical: dict[str, Any]) -> dict[str, Any]:
        missing = []
        if "bilirubin" not in labs:
            missing.append("bilirubin")
        if "albumin" not in labs:
            missing.append("albumin")
        if "inr" not in labs:
            missing.append("inr")
        if "ascites" not in clinical:
            missing.append("ascites")
        if "encephalopathy" not in clinical:
            missing.append("encephalopathy")
    ...
    @staticmethod
    def malaria_severity(
        vitals: dict[str, Any], labs: dict[str, Any], clinical: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    @staticmethod
    def diabetes_risk_hba1c(
        hba1c: float | None = None, fpg: float | None = None
    ) -> dict[str, Any]:
...
def _compute_patient_scores(patient_state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if vitals and all(
        k in vitals and vitals[k] is not None
        for k in ["rr", "spo2", "bp_systolic", "heart_rate", "temperature"]
    ):
        try:
    ...
    if "hiv" in [c.lower() for c in active_conditions]:
        ...
        try:
            ...
            alert = "WARNING" if who_res["stage"] >= 3 else "INFO"
    ...
    # Check Malaria
    if "malaria" in [c.lower() for c in active_conditions] or any(
        "malaria" in d.get("condition_name", "").lower()
        for d in patient_state.get("active_diagnoses", [])
    ):
        lab_dict = {k: v["value"] for k, v in labs.items()}
        try:
            mal_res = ClinicalScorer.malaria_severity(vitals, lab_dict, clinical_params)
            alert = "CRITICAL" if mal_res["is_severe"] else "INFO"
