...
class TableImporter:
    ...
    """
    def __init__(self, db_path: str = "app/data/cdss_index", validated_dir: str = "app/kb/validated"):
    ...
    def import_disease_tables(self, disease: str):
        ...
        for f_path in disease_dir.glob("*.json"):
            ...
            for i, row in enumerate(table_data.get("data", [])):
                nl_text = self._render_row_to_text(row, f"{disease.upper()} {t_type.replace('_', ' ').title()} Table")
        ...
        if num_rows > 100:
            logger.info(f"Creating IVF-PQ index: partitions={num_partitions}, sub_vectors={num_sub_vectors}")
            table.create_index(metric="cosine", vector_column_name="vector", num_partitions=num_partitions, num_sub_vectors=num_sub_vectors)
