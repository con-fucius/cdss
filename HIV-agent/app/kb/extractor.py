...
class TableExtractor:
    ...
    def _classify_table(self, headers: List[str], text_content: str) -> str:
        ...
        if any(w in headers_str for w in ["regimen", "drug", "arv", "medication", "first-line", "second-line", "act", "artemether", "artesunate", "lumefantrine"]):
            return "regimen"
        if any(w in headers_str for w in ["dose", "weight", "dosing", "kg", "mg", "tablet", "body weight"]):
            return "dosing"
        if any(w in headers_str for w in ["criteria", "threshold", "fpg", "hba1c", "target", "parasitemia", "rdt", "smear"]):
            return "diagnostic_criteria"
        if any(w in headers_str for w in ["monitoring", "schedule", "follow-up", "month", "day"]):
            return "monitoring"
        return "reference_values"

    def _compute_quality_score(self, table_type: str, num_cols: int, num_rows: int, headers: List[str]) -> Dict[str, Any]:
        ...
        if table_type == "regimen":
            ...
            if any("value" in h or "target" in h or "threshold" in h or "criteria" in h for h in headers_lower):
                score["expected_headers_present"] = True
