...
class TableValidator:
    ...
    def __init__(
        self,
        raw_dir: str = "app/kb/raw",
        validated_dir: str = "app/kb/validated",
        flagged_dir: str = "app/kb/flagged",
    ):
    ...
    def validate_table(self, table_data: dict[str, Any]) -> bool:
        ...
        if t_type == "regimen":
            ...
            has_dose = any(
                col for col in headers if "dose" in col or "tablet" in col or "mg" in col
            )
            has_range = any(
                col
                for col in headers
                if "weight" in col or "age" in col or "kg" in col or "year" in col
            )
