...
if TYPE_CHECKING:
    from .api import DDxRequest

from .repositories import query_evidence_graph_db
from .search_tools import SearchIndex
from .providers import get_llm_provider, provider_offline, provider_chat_endpoint, provider_auth_header
...
class DifferentialDiagnosisEngine:
    ...
    async def generate_ddx(self, request: Any, search_index: SearchIndex) -> AsyncIterator[Dict]:
        ...
        for symptom in request.presenting_symptoms:
            for disease in target_diseases:
                ...
                for edge in edges:
                    ...
                    if "suggest" in rel or "indicat" in rel or "manifest" in rel or "symptom" in rel:
        ...
        # Sort and take top 5
        sorted_candidates = sorted(list(candidates.values()), key=lambda x: x["weight"], reverse=True)[:5]
        ...
        for cand in sorted_candidates:
            ...
            results = await search_index.query_pageindex(
                query=f"{condition} diagnostic criteria",
                disease=disease,
                top_k=2
            )
        ...
        if provider_offline():
            yield {
                "type": "warning",
                "message": "Offline mode active. LLM synthesis skipped."
            }
