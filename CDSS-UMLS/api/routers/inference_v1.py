    ...
    try:
        ...
        if request.use_rag:
            try:
                ...
                logger.info(f"RAG retrieved {len(context)} context documents and {len(umls_concepts)} CUIs")
    ...
    try:
        ...
        return {
            ...
            "direct_search_sample": [
                {
                    "cui": r.get("payload", {}).get("cui", "") if isinstance(r, dict) else (r.payload.get("cui", "") if hasattr(r, 'payload') else ""),
                    "name": (r.get("payload", {}).get("preferred_name", "") if isinstance(r, dict) else (r.payload.get("preferred_name", "") if hasattr(r, 'payload') else ""))[:50],
                    "score": float(r.get("score", 0.0) if isinstance(r, dict) else (r.score if hasattr(r, 'score') else 0.0))
                }
        ...
        return {
            ...
            "rag_service_initialized": rag_service.client is not None and rag_service.embedding_model is not None
        }
