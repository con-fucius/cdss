...
class OpenAIModel:
    ...
    def __init__(self, model_name: str = "gpt-4"):
        ...
        if api_key.startswith("sk-"):
            logger.info(f"OpenAI API key loaded (length: {len(api_key)}, starts with: {api_key[:10]}...)")
        else:
            logger.warning(f"OpenAI API key format unexpected (starts with: {api_key[:10] if len(api_key) > 10 else api_key}...)")
