...
class RAGServiceQdrant(BaseRAGService):
    ...
    def _initialize(self):
        ...
        try:
            ...
            try:
                ...
                if not collection_exists:
                    ...
                    try:
                        ...
                        logger.info(
                            f"✓ Connected to Qdrant. Collection '{self.collection_name}' has {collection_info.points_count:,} points"
                        )
                    except Exception as info_error:
                        # Version mismatch - collection exists but can't read full info
                        logger.warning(
                            f"Collection '{self.collection_name}' exists but couldn't read info (version mismatch): {info_error}"
                        )
                        logger.info(
                            f"✓ Connected to Qdrant. Collection '{self.collection_name}' is available"
                        )
            ...
            self.embedding_model = None
            logger.warning(
                "Qdrant RAG service initialization failed - RAG will not work until fixed"
            )
    ...
    async def _get_relations(
        self, cuis: list[str], relation_types: list[str] | None = None
    ) -> list[dict[str, str]]:
    ...
    async def _get_drug_interactions(self, drug_cuis: list[str]) -> list[dict[str, str]]:
    ...
    async def retrieve(
        ...
        try:
                ...
                if not full_query_results and hasattr(client, "query_points"):
                    try:
                        ...
                        query_result = client.query_points(
                            collection_name=self.collection_name, query=query_obj
                        )
                        full_query_results = (
                            query_result.points if hasattr(query_result, "points") else []
                        )
                ...
                for result in full_query_results:
                    ...
                    if cui:
                        ...
                        preferred_name = (
                            ...
                            if isinstance(payload, dict)
                            else getattr(payload, "preferred_name", "")
                        )
                        semantic_types = (
                            payload.get("semantic_types", [])
                            if isinstance(payload, dict)
                            else getattr(payload, "semantic_types", [])
                        )
                        text = (
                            payload.get("text", "")
                            if isinstance(payload, dict)
                        ...
                        logger.debug(
                            f"Added full query result: CUI {cui} ({preferred_name[:50] if preferred_name else 'N/A'})"
                        )
            except Exception as e:
            ...
            for entity in entities:
                try:
                    ...
                    entity_embedding = self.embedding_model.encode(
                        [entity], show_progress_bar=False
                    )[0]
                    ...
                    for result in search_results:
                        ...
                        if cui:
                            ...
                            if cui not in [r["source"].replace("CUI:", "") for r in all_results]:
                                ...
                                logger.debug(
                                    f"Added entity result: CUI {cui} ({preferred_name[:50] if preferred_name else 'N/A'})"
                                )
                except Exception as e:
            ...
            for result in all_results:
                ...
                if cui and (
                    cui not in results_dict
                    or float(result["score"]) > float(results_dict[cui]["score"])
                ):
                    results_dict[cui] = result

            results = list(results_dict.values())[
                : top_k * 2
            ]  # Get more for relationship expansion
            umls_concepts = list(all_cuis)

            logger.info(
                f"After similarity search: {len(results)} results, {len(umls_concepts)} unique CUIs"
            )
            ...
            if not results:
                ...
                logger.warning(
                    "This might indicate: 1) Query doesn't match indexed text, 2) Embedding mismatch, or 3) Collection issue"
                )
            ...
            try:
                ...
                drug_cuis = [
                    cui
                    for cui in umls_concepts
                    if cui in concept_details
                    and self._is_drug(concept_details[cui].get("semantic_types", []))
                ]
                ...
                logger.warning(
                    f"Error retrieving drug interactions (continuing anyway): {drug_err}"
                )
            ...
            logger.info(
                f"Retrieved {len(results)} total results ({len(umls_concepts)} concepts, {len(relations)} relations, {len(drug_cuis)} drugs)"
            )
            ...
            if not results and all_results:
                logger.warning(
                    "No results after processing, but had basic search results. Returning basic results."
                )
    ...
    async def index_documents(self, documents: list[dict[str, str]]):
        ...
        """
        logger.warning(
            "index_documents called on Qdrant RAG service. Use generate_embeddings_qdrant.py to index documents."
        )
        pass
