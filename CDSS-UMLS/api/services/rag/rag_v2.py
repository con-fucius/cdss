...
class RAGServiceV2(BaseRAGService):
    ...
    async def retrieve(
        self, query: str, top_k: int = 5, clinical_context_window: int | None = None, **kwargs
        ...
        try:
            ...
            result = db.execute(sql, {"query_embedding": str(query_embedding), "top_k": top_k})
            ...
            for row in result:
                ...
                if clinical_context_window:
                    relations = await self._get_semantic_relations(
                        row.umls_concepts, clinical_context_window
                    )
    ...
    async def _get_semantic_relations(
        self, cuis: list[str], window_size: int
    ) -> list[dict[str, str]]:
