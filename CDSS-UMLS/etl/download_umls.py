...
def get_tgt(api_key):
    if not api_key:
        raise ValueError(
            "UMLS_API_KEY is not set. Please set UMLS_API_KEY in your .env file."
        )
    url = "https://utslogin.nlm.nih.gov/cas/v1/api-key"
    resp = requests.post(url, data={"apikey": api_key})
    if resp.status_code != 201:
        raise Exception(
            f"Failed to get TGT. Status code: {resp.status_code}, "
...
def download_umls():
    ...
    uts_download_url = f"https://uts-ws.nlm.nih.gov/download?url={base_download_url}&apiKey={UMLS_API_KEY}"
    
    print(f"\n{'='*60}")
    print(f"Method 1: UTS Download API (Recommended)")
    print(f"{'='*60}")
    ...
    try:
        ...
        with requests.get(uts_download_url, stream=True, timeout=(30, 1800), 
            ...
            if r.status_code in [200, 206]:
                ...
                if resume_pos > 0 and r.status_code == 206:
                    ...
                    print(f"✓ Server supports resume (HTTP 206)")
                    mode = "ab"
                elif resume_pos > 0 and r.status_code == 200:
                    print(f" Server doesn't support resume, re-downloading from start...")
                    resume_pos = 0
                    mode = "wb"
                else:
                    mode = "wb"
                ...
                with open(file_path, mode) as f, tqdm(total=total, initial=resume_pos, 
                                                      unit="B", unit_scale=True, desc="Downloading") as bar:
                ...
                if total and actual_size != total:
                    if actual_size < total * 0.9:
                        raise Exception(f"Download incomplete. Expected {total} bytes, got {actual_size} bytes")
                    else:
                ...
                try:
                    with zipfile.ZipFile(file_path, 'r') as test_zip:
                        ...
                        rrf_count = len([f for f in test_zip.namelist() if f.upper().endswith('.RRF')])
                        print(f"✓ Zip file is valid")
                        print(f"  Total files: {file_count}")
                        print(f"  RRF files: {rrf_count}")
                        
                        required_rrf = ['MRCONSO.RRF', 'MRREL.RRF', 'MRSTY.RRF']
                        found_rrf = [f.upper().split('/')[-1] for f in test_zip.namelist() if f.upper().endswith('.RRF')]
                        missing = [f for f in required_rrf if f not in found_rrf]
                        
                        if missing:
                            print(f"⚠ Warning: Missing expected RRF files: {missing}")
                        else:
                            print(f"✓ All required RRF files found")
                except zipfile.BadZipFile:
                    raise Exception("Downloaded file is not a valid zip file. Download may be corrupted.")
        ...
        print(f"Falling back to CAS ticket authentication method...")
    
    # Method 2: Fallback to CAS ticket authentication
    print(f"\n{'='*60}")
    print(f"Method 2: CAS Ticket Authentication (Fallback)")
    print(f"{'='*60}")
    ...
    for idx, download_url in enumerate(urls_to_try):
        try:
            encoded = idx > 0
            print(f"Attempt {idx + 1}/{len(urls_to_try)}: Trying {'URL-encoded' if encoded else 'standard'} ticket format...")
            ...
            print(f"Starting download (timeout: 30 minutes for large files)...")
            with requests.get(download_url, stream=True, timeout=(30, 1800), allow_redirects=True) as r:
                if r.status_code == 200:
                    total = int(r.headers.get("content-length", 0))
                    
                    if total == 0:
                        print(f"Warning: Content-Length is 0, trying next format...")
                        continue
                    ...
                    if total < 500 * 1024 * 1024:  # Less than 500MB is suspicious
                        print(f"⚠ Warning: File size ({size_mb:.2f} MB) seems too small for full UMLS dataset.")
                        print(f"Expected: 5-10GB+. This might be a partial download.")
                    ...
                    if os.path.exists(file_path):
                        ...
                        if resume_pos > 0 and resume_pos < total:
                            ...
                            print(f"✓ File already exists and appears complete ({resume_pos / (1024**2):.2f} MB)")
                    ...
                    print(f"  Expected full UMLS dataset size: 5-10GB+")
                    ...
                    resume_r = requests.get(download_url, stream=True, timeout=(30, 1800), 
                    ...
                    if actual_size != total and total > 0:
                        print(f"Warning: Download size mismatch. Expected: {total}, Got: {actual_size}")
            ...
            print(f"⚠ Request timed out. This is common for large downloads.")
            print(f"   The service ticket may have expired. Will try with fresh ticket...")
            # Try next URL format
            continue
            ...
            if "timeout" in error_str or "timed out" in error_str:
                ...
                print(f"   Will try with fresh ticket on next attempt...")
                continue
            else:
        ...
        if last_error and ("timeout" in last_error.lower() or "timed out" in last_error.lower()):
            ...
            try:
                ...
                if fresh_st:
                    ...
                    print(f"Retrying download with fresh ticket (longer timeout: 30 minutes)...")
                    try:
                        with requests.get(retry_url, stream=True, timeout=(30, 1800), allow_redirects=True) as r:
                            if r.status_code == 200:
                                total = int(r.headers.get("content-length", 0))
                                if total > 0:
                                    size_gb = total / (1024**3)
...
def download_umls_with_library():
    ...
    try:
        ...
        raise Exception("umls_downloader library not available. Install with: pip install umls-downloader")
    except Exception as e:
        ...
        try:
            ...
            with requests.get(base_download_url, headers=headers, stream=True, timeout=300) as r:
                if r.status_code == 200:
                    total = int(r.headers.get("content-length", 0))
                    if total > 0:
                        print(f"Success with header authentication! Downloading {total / (1024*1024):.2f} MB...")
...
if __name__ == "__main__":
    ...
    if not zip_path:
        try:
            ...
            print(f"✗ API key header method failed")
