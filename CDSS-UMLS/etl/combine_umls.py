...
def parse_rrf_file(file_path: Path, field_separator: str = "|") -> List[List[str]]:
    ...
    for file_to_parse in files_to_parse:
        ...
        with open(file_to_parse, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            desc = f"  Parsing {file_to_parse.name}" if len(files_to_parse) > 1 else f"  Parsing {file_path.name}"
            reader = tqdm(
                reader, 
                desc=desc, 
                unit=" lines", 
    ...
    if len(files_to_parse) > 1:
        logger.info(f"  ✓ Parsed {len(records):,} records from {len(files_to_parse)} files ({total_size_mb:.1f} MB total)")
    else:
    ...
    for record in tqdm(records, desc="MRCONSO"):
        ...
        if is_pref and string:
            ...
            # If we have a preferred name, check if this source is higher priority
                ...
                current_priority = SOURCE_PRIORITY.index(source) if current_source in SOURCE_PRIORITY else 999
...
def load_mrdef(records: List[List[str]], allowed_sources: List[str] = None) -> Dict[str, List[Dict]]:
    ...
    if allowed_sources is not None:
        ...
        logger.info(f"  No source filtering applied (all sources included)")
...
def stream_mrrel_to_csv(file_path: Path, output_file: Path) -> int:
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as out_f:
        writer = csv.DictWriter(out_f, fieldnames=["cui1", "cui2", "relation", "relation_label", "source"])
        writer.writeheader()
        ...
        for file_to_parse in files_to_parse:
            with open(file_to_parse, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                desc = f"  Streaming {file_to_parse.name}" if len(files_to_parse) > 1 else f"  Streaming {file_path.name}"
                reader = tqdm(
                    reader,
                    desc=desc,
                    unit=" lines",
...
def load_mrhier_from_file(file_path: Path) -> Dict[str, List[Dict]]:
    ...
    for file_to_parse in files_to_parse:
        with open(file_to_parse, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            desc = f"  Processing {file_to_parse.name}" if len(files_to_parse) > 1 else f"  Processing {file_path.name}"
            reader = tqdm(
                reader,
                desc=desc,
                unit=" lines",
...
def load_mrsat_from_file(file_path: Path) -> Dict[str, List[Dict]]:
    ...
    logger.info(f"  MRSAT Processing Summary:")
...
def combine_concepts(
    ...
    for cui, concept_data in tqdm(concepts.items(), desc="Combining"):
        ...
        if not preferred_name or preferred_name in {'N', '0', '3', '256', '9', ''}:
            ...
            PLACEHOLDER_VALUES = {'N', '0', '3', '256', '9', ''}
            meaningful_synonyms = [s for s in concept_data.get("synonyms", []) 
                                 if s and s not in PLACEHOLDER_VALUES and len(s) > 2]
            preferred_name = meaningful_synonyms[0] if meaningful_synonyms else (concept_data["synonyms"][0] if concept_data["synonyms"] else "")
        ...
        def_list = [d for d in def_list if d and isinstance(d, dict) and d.get("definition", "").strip()]
        ...
        combined[cui] = {
            ...
            "synonyms": concept_data["synonyms"] if isinstance(concept_data["synonyms"], list) else [],
...
def main():
    ...
    parser.add_argument("--skip-sat", action="store_true",
                       help="Skip MRSAT (additional attributes) to save memory")
    ...
    mrrel_records = parse_rrf_file(mrrel_path)
    ...
    relations = []  # Empty list - relations are already written to CSV
    ...
    combined_concepts = combine_concepts(concepts, semantic_types, definitions, hierarchies, attributes)
    ...
    if relation_count > 0:
        logger.info(f"✓ Relations already written to {relations_file.name} ({relation_count:,} relations)")
    else:
    ...
    if relation_count > 0 and relations_file.exists():
        with open(relations_file, 'r', encoding='utf-8') as in_f, \
             open(neo4j_relations_file, 'w', newline='', encoding='utf-8') as out_f:
            reader = csv.DictReader(in_f)
            writer = csv.DictWriter(out_f, fieldnames=[
                ":START_ID(Concept)", ":END_ID(Concept)", ":TYPE", "relation_label", "source"
            ])
            writer.writeheader()
    ...
    with open(neo4j_concepts_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=[
            ":ID(Concept)", "preferred_name", "semantic_types", "definition", "synonym_count"
        ])
        writer.writeheader()
    ...
    # File sizes
    logger.info("\nGenerated files:")
    for file in [concepts_file, relations_file, embeddable_file, neo4j_relations_file, neo4j_concepts_file]:
        if file.exists():
