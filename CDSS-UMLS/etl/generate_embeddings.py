...
def generate_embeddings(jsonl_path: Path, batch_size: int = 128, update_batch_size: int = 1000):
        ...
        for line_num, line in enumerate(tqdm(f, total=total_lines, desc="Processing"), 1):
            try:
                ...
                if len(batch_texts) >= batch_size:
                    ...
                    # Add to update batch
                    for cui_item, embedding in zip(batch_cuis, embeddings, strict=False):
        ...
        with engine.begin() as conn:  # Use begin() for automatic transaction management
            ...
            if result.fetchone() is None:
                ...
                if count >= 1000:
                    ...
                    logger.info(
                        f"Not enough embeddings ({count}) for ivfflat index. Will create later."
                    )
    except Exception as e:
