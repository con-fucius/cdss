...
def generate_embeddings_to_qdrant(
    jsonl_path: Path, batch_size: int = 128, upload_batch_size: int = 1000
):
    ...
    if next_id > 0:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(cui_to_id):,} existing CUI->ID mappings from {loaded_count:,} points"
            )
        except Exception as e:
            logger.warning(
                f"Could not load existing mappings: {e}. Will assign new IDs (may cause duplicates if CUIs already exist)."
            )
    ...
    with open(jsonl_path, encoding="utf-8") as f:
        for line_num, line in enumerate(tqdm(f, total=total_lines, desc="Processing"), 1):
            try:
                ...
                if len(batch_texts) >= batch_size:
                    ...
                    for (cui, text, full_data), embedding in zip(batch_data, embeddings, strict=False):
                        ...
                        point = PointStruct(
                            ...
                            payload={
                                ...
                                "codes": full_data.get(
                                    "codes", []
                                ),  # Codes stored as metadata (not embedded)
                            },
                        )
                    ...
                    if len(upload_batch) >= upload_batch_size:
                        try:
                            client.upsert(
                                collection_name=settings.QDRANT_COLLECTION_NAME, points=upload_batch
                            )
        ...
        if batch_texts:
            embeddings = model.encode(
                batch_texts, show_progress_bar=False, batch_size=batch_size, convert_to_numpy=True
            )
        ...
        if upload_batch:
            try:
                client.upsert(collection_name=settings.QDRANT_COLLECTION_NAME, points=upload_batch)
                uploaded_count += len(upload_batch)
            except Exception as e:
